...
module.exports = function(overideSettings) {
    ...
    if (settings.appCredentials.scope.indexOf('inventory') != -1) {
        var inventoryScope = {
            products: {
                ...
                patch: {
                    ...
                    attributes: {
                        $: function(productId, name, query, cb) {
                            ...
                            var opp = 'Attributes';
                            if (name) opp += '(' + name + ')';
                            makeRequests('PATCH', endpoint, opp, query, cb);
                        }
                    },
                    images: {
                        $: function(productId, placementName, query, cb) {
                    ...
                    dCQuantities: {
                        $: function(productId, dcID, query, cb) {
                            var endpoint = 'Products(' + productId + ')';
                            var opp = 'DCQuantities';
                            if (dcID) opp += '(' + dcID + ')';
                            makeRequests('PATCH', endpoint, opp, query, cb);
                        }
                    }
                ...
                del: {
                    attributes: {
                        $: function(productId, name, cb) {
                            var endpoint = 'Products(' + productId + ')';
                            var opp = 'Attributes';
                            if (name) opp += '(' + name + ')';
                            makeRequests('DELETE', endpoint, opp, null, cb);
                        }
                    },
                    images: {
                        $: function(productId, placementName, cb) {
                            var endpoint = 'Products(' + productId + ')';
                            var opp = 'images';
                            if (placementName) opp += '(' + placementName + ')';
                            makeRequests('DELETE', endpoint, opp, null, cb);
                    ...
                    bundleComponents: {
                        $: function(productId,componentID, query, cb) {
                            ...
                            var opp = 'BundleComponents';
                            if (componentID) opp += '(' + componentID + ')';
                            makeRequests('DELETE', endpoint, opp, query, cb);
                        }
                    },
                    dCQuantities: {
                        $: function(id, cb) {
                            var endpoint = 'Products';
                            if (id) endpoint += '(' + id + ')';
                            makeRequests('DELETE', endpoint, null, null, cb);
                        }
                    }
    ...
    function init(options){
        ...
        currentTokken = null;
        tokenProvider = new TokenProvider(settings.apiEndPoint + "oauth2/token", {
    ...
    function makeRequests(type, endpoint, operation, query, callback) {
        ...
        getToken(function(token) {
            var params = {
                ...
                body: query, // not used in get
                qs: query, // for get commands
                json: true
            };
            switch (type) {
                case 'GET':
                    request.get(params, function(err, request, body) {
                        ...
                        debugLog("Body: ", body);
                        if (request.statusCode === 401) return callback('ACCESS DENIED', request);
                ...
                case 'PATCH':
                    request.patch(params, function(err, request, body) {
                        ...
                        debugLog("Request: ", request);
                        debugLog("Body: ", body);
                        if (request.statusCode === 401) return callback('ACCESS DENIED');
                        return callback(err, body);
                    });
                    break;
                case 'PUT':
